...
vec4 map(vec3 p){
    float plane = dot(p,vec3(0.0,1.0,0.0))+1.728;//vec3 Rotation
    ...
    // Scene
    vec4 scene = vec4(0.);
    scene = unionSDF(octWithsph,sphere);
...
vec3 diffuseLighting(vec3 p,vec3 c){
    ...
    vec3 lightPosition = vec3(0,3.,-3.5);
    //vec3 lightPosition = 1.2*vec3( 3.5*cos(an),2.1,  3.5*sin(an) );
...
float GetLight(vec3 p){
    ...
    float d = RayMarch(p+n*MIN_DISTANCE*2.,l,difColor);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    // camera movement	
	float an = 0.5*(iTime-10.0);
	vec3 ro = 1.2*vec3( 3.5*cos(an), 2.0, 3.5*sin(an) );
    
    // float an = -1.5;
    //vec3 ro = vec3(0,1.0,-4.5);
    ...
    for( int n=0; n<AA; n++ )
    {
        ...
        for( int i=0; i<256; i++ )
        {
            ...
            vec3 pos = ro + t*rd;
            float h = map(pos).a;
            if( h<0.0001 || t>T_MAX ) break;//CHANGE TO T_MAX TO SKYBOX
        ...
        if( t<tmax )//T_MAX
        {
            ...
            float amb = .5 + .5*dot(nor,vec3(0.0,1.0,0.0));
